...
public class DatabaseLoader {
    ...
    public CommandLineRunner initData(ProductRepository repository, CategoryRepository categoryRepository) {
        return args -> {
            // 공통 옵션 생성
            Option commonOption = new Option("포장 유무", 100, 0, null); // 수량 100, 가격 0, 실제 상품에 설정될 때 제품 연결 필요
            ...
            List<Option> options1 = Arrays.asList(
                new Option("01. [Best] 시어버터 핸드 & 시어 스틱 립 밤", 1, 15000, product1),
                new Option("02. [Special] 더블 체다 치즈볼", 1, 12000, product1),
                new Option(commonOption.getName(), commonOption.getQuantity(), commonOption.getPrice(), product1)
            );
            product1.setOptions(options1);
            repository.save(product1);

            Product product2 = new Product(2002L, "스타벅스 기프트 카드 10000원", 10000, "https://image.istarbucks.co.kr/cardImg/20200818/007633_WEB.png", 2);
            List<Option> options2 = Arrays.asList(
                new Option("01. [Gift] 스타벅스 아메리카노", 1, 5000, product2),
                new Option("02. [Gift] 스타벅스 라떼", 1, 5500, product2),
                new Option(commonOption.getName(), commonOption.getQuantity(), commonOption.getPrice(), product2)
            );
